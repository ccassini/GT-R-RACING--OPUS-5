...
export function createEngineVoice(ctx, destination, noiseBuffer) {
  ...
  // ── NOS / nitro — intake rush + forced-induction whoosh (procedural) ──
  const nosIntakeSrc = ctx.createBufferSource();
  nosIntakeSrc.buffer = noiseBuffer;
  nosIntakeSrc.loop = true;
  const nosIntakeHP = ctx.createBiquadFilter();
  nosIntakeHP.type = 'highpass';
  nosIntakeHP.frequency.value = 260;
  const nosIntakeBP = ctx.createBiquadFilter();
  nosIntakeBP.type = 'bandpass';
  nosIntakeBP.frequency.value = 1050;
  nosIntakeBP.Q.value = 0.9;
  const nosIntakeGain = ctx.createGain();
  nosIntakeGain.gain.value = 0;
  nosIntakeSrc.connect(nosIntakeHP);
  nosIntakeHP.connect(nosIntakeBP);
  nosIntakeBP.connect(nosIntakeGain);
  nosIntakeGain.connect(output);
  nosIntakeSrc.start();

  const nosWhooshSrc = ctx.createBufferSource();
  nosWhooshSrc.buffer = noiseBuffer;
  nosWhooshSrc.loop = true;
  const nosWhooshHP = ctx.createBiquadFilter();
  nosWhooshHP.type = 'highpass';
  nosWhooshHP.frequency.value = 160;
  const nosWhooshLP = ctx.createBiquadFilter();
  nosWhooshLP.type = 'lowpass';
  nosWhooshLP.frequency.value = 880;
  const nosWhooshGain = ctx.createGain();
  nosWhooshGain.gain.value = 0;
  nosWhooshSrc.connect(nosWhooshHP);
  nosWhooshHP.connect(nosWhooshLP);
  nosWhooshLP.connect(nosWhooshGain);
  nosWhooshGain.connect(output);
  nosWhooshSrc.start();
  ...
  return {
    ...
    update(state, dt) {
      ...
      const boostTarget = state.boost ? 1 : 0;
      const boostRate = state.boost ? 9.5 : 5.5;
      smoothBoost += (boostTarget - smoothBoost) * Math.min(1, boostRate * dt);

      const load = throttle ? 0.78 + rpmFrac * 0.28 : 0.18 + rpmFrac * 0.14;
      const boostLift = 1 + smoothBoost * 0.24;
      ...
      const lpOpen =
        360 + rpmFrac * character.brightness * (1 - redline * 0.6) + smoothBoost * 160;
      const lpCap = 400 + (1 - redline) * 350;
      engineLP.frequency.linearRampToValueAtTime(Math.min(lpOpen, lpCap), now + ramp);

      exhaustLP.frequency.linearRampToValueAtTime(
        80 + rpmFrac * 280 * (1 - redline * 0.3) + smoothBoost * 90,
        now + ramp,
      );
      exhaustBP.frequency.linearRampToValueAtTime(
        160 + rpmFrac * 220 * (1 - redline * 0.35) + smoothBoost * 60,
        now + ramp,
      );
      const exhaust =
        ...
        character.exhaust *
        (1 - redline * 0.2);
      const exhaustSurge = 1 + smoothBoost * 0.52;
      exhaustGain.gain.linearRampToValueAtTime(exhaust * drift * exhaustSurge, now + ramp);

      // NOS intake rush — bandpassed air charge, not a synth sweep.
      const intakeHz = 620 + rpmFrac * 1300 + smoothBoost * 180;
      nosIntakeBP.frequency.linearRampToValueAtTime(intakeHz, now + ramp);
      nosIntakeBP.Q.linearRampToValueAtTime(0.75 + smoothBoost * 0.45, now + ramp);
      const intakeLevel =
        smoothBoost * (0.042 + rpmFrac * 0.052 + (throttle ? 0.018 : 0));
      nosIntakeGain.gain.linearRampToValueAtTime(intakeLevel * drift, now + ramp);

      // Forced-induction whoosh — mid-band turbulent rush layered under the V12.
      nosWhooshLP.frequency.linearRampToValueAtTime(
        480 + rpmFrac * 620 + smoothBoost * 280,
        now + ramp,
      );
      nosWhooshHP.frequency.linearRampToValueAtTime(130 + rpmFrac * 110, now + ramp);
      const whooshLevel = smoothBoost * (0.028 + rpmFrac * 0.038);
      nosWhooshGain.gain.linearRampToValueAtTime(whooshLevel * drift, now + ramp);
      ...
      const turbo =
        turboAllowed && rpmFrac > 0.55
          ...
          : 0;
      turboGain.gain.linearRampToValueAtTime(turbo, now + ramp);

      if (wasBoost && !state.boost && rpmFrac > 0.38 && Math.random() > 0.62) {
        pop(0.28 + rpmFrac * 0.22);
      }
      wasBoost = state.boost;
    ...
    reset() {
      ...
      nosIntakeGain.gain.linearRampToValueAtTime(0, now + 0.06);
      nosWhooshGain.gain.linearRampToValueAtTime(0, now + 0.06);
    },
