...
export function createEngineVoice(ctx, destination, noiseBuffer) {
  ...
  return {
    ...
    update(state, dt) {
      ...
      // At redline, favour sub/mid body over upper harmonics so the
      // needle hitting the stop stays a thick V12 roar, not a whine.
      const redline = Math.max(0, (rpmFrac - 0.72) / 0.28);
      const upperCut = 1 - redline * 0.7;
      ...
      for (let i = 0; i < ORDER_COUNT; i++) {
        ...
        const harmScale = order.mult >= 2 ? upperCut : 1 + redline * (order.mult <= 0.5 ? 0.35 : 0.12);
      ...
      growlBP.frequency.linearRampToValueAtTime(320 + rpmFrac * 480, now + ramp);
      const growlLevel =
        (throttle ? 0.06 + rpmFrac * 0.1 : 0.012 + rpmFrac * 0.02) * character.growl;
      growlGain.gain.linearRampToValueAtTime(growlLevel * drift * (1 + redline * 0.25), now + ramp);
      ...
      // Cap the lowpass so redline never opens into a piercing band.
      engineLP.frequency.linearRampToValueAtTime(
        380 + rpmFrac * character.brightness * (1 - redline * 0.35),
        now + ramp,
      );

      exhaustLP.frequency.linearRampToValueAtTime(80 + rpmFrac * 320, now + ramp);
      exhaustBP.frequency.linearRampToValueAtTime(180 + rpmFrac * 280, now + ramp);
      const exhaust =
      ...
      // Induction whisper only when character.turbo > 0; muscle V12 stays silent.
